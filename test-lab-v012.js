(() => {
  ...
  const CACHE_LABEL = "meet-schwerin-v0.12.0-r1";
  ...
  function ensureUi() {
    let banner = document.getElementById("v012TestBanner");
    if (!banner) {
      banner = document.createElement("div");
      banner.id = "v012TestBanner";
      banner.className = "v012-banner";
      banner.innerHTML = `<strong>🧪 TEST MODE</strong><span id="v012BannerTime"></span><button type="button" id="v012OpenLab">Open Test Lab</button>`;
      document.body.prepend(banner);
      banner.querySelector("#v012OpenLab")?.addEventListener("click", () => {
        document.getElementById("v012TestLab")?.scrollIntoView({ behavior: "smooth", block: "start" });
      });
    }

    let panel = document.getElementById("v012TestLab");
    if (panel) return panel;
    panel = document.createElement("section");
    panel.id = "v012TestLab";
    panel.className = "v012-lab";
    panel.innerHTML = `
      <div class="v012-head">
        ...
        <div class="v012-head-actions"><button type="button" id="v012Reset">Reset simulation</button><button type="button" class="danger" id="v012Disable">Exit Test Mode</button></div>
      </div>
      <div class="v012-tabs" role="tablist" aria-label="Test Lab sections">
        <button type="button" class="active" data-v012-tab="clock">Clock</button>
        <button type="button" data-v012-tab="people">People</button>
        <button type="button" data-v012-tab="network">Network</button>
        <button type="button" data-v012-tab="diag">Diagnostics</button>
      </div>
      <div class="v012-pane active" data-v012-pane="clock">
        <div class="v012-clock-hero"><div><small>SIMULATED NOW</small><strong id="v012ClockNow">—</strong><em id="v012ClockReal">Real: —</em></div><span id="v012ClockSpeed">1×</span></div>
        <label class="v012-field"><span>Set simulated date & time</span><input id="v012DateTime" type="datetime-local" step="1"></label>
        <div class="v012-grid-buttons" id="v012Nudges">
        ...
        <label class="v012-field"><span>Time speed</span><select id="v012Speed"><option value="0">Paused</option><option value="1">1×</option><option value="5">5×</option><option value="30">30×</option><option value="60">60×</option></select></label>
        <div class="v012-events"><div class="v012-section-title"><strong>Jump to route event</strong><small>Generated from the current Best recommendation</small></div><div id="v012EventButtons" class="v012-event-buttons"></div><div id="v012ScrubberWrap"></div></div>
      </div>
      <div class="v012-pane" data-v012-pane="people">
        <div class="v012-section-title"><strong>Simulated people</strong><small>These states never reach Cloudflare KV.</small></div>
        <div id="v012People"></div>
      </div>
      <div class="v012-pane" data-v012-pane="network">
        <div class="v012-section-title"><strong>Provider / network faults</strong><small>Applied only while Test Mode is enabled.</small></div>
        <label class="v012-field"><span>Network scenario</span><select id="v012Network">
        ...
        <div class="v012-network-note" id="v012NetworkNote"></div>
        <button type="button" class="v012-primary" id="v012TestReplan">Run Refresh & replan</button>
      </div>
      <div class="v012-pane" data-v012-pane="diag">
        <div class="v012-section-title"><strong>Diagnostics</strong><small>Useful when something behaves differently on a phone.</small></div>
        <div class="v012-diag" id="v012Diagnostics"></div>
        <button type="button" id="v012CopyDiagnostics">Copy diagnostics</button>
      </div>`;
    ...
    panel.querySelectorAll("[data-v012-tab]").forEach((button) => {
      button.addEventListener("click", () => {
        panel.querySelectorAll("[data-v012-tab]").forEach((item) => item.classList.toggle("active", item === button));
        panel.querySelectorAll("[data-v012-pane]").forEach((pane) => pane.classList.toggle("active", pane.dataset.v012Pane === button.dataset.v012Tab));
        if (button.dataset.v012Tab === "diag") renderDiagnostics();
      });
    });
    panel.querySelector("#v012Disable")?.addEventListener("click", () => setEnabled(false));
    panel.querySelector("#v012Reset")?.addEventListener("click", resetSimulation);
    panel.querySelector("#v012DateTime")?.addEventListener("change", (event) => {
    ...
    panel.querySelector("#v012Speed")?.addEventListener("change", (event) => setSpeed(Number(event.target.value)));
    panel.querySelector("#v012Network")?.addEventListener("change", (event) => {
      state.network = event.target.value;
      saveState();
      dispatchState();
    });
    panel.querySelector("#v012TestReplan")?.addEventListener("click", () => window.NVSLiveMeetup?.refresh?.());
    panel.querySelector("#v012CopyDiagnostics")?.addEventListener("click", copyDiagnostics);
    return panel;
  }

  function renderPeople() {
    const root = document.getElementById("v012People");
    if (!root) return;
    const list = members();
    if (!list.length) {
      root.innerHTML = `<p class="v012-empty">Find a route first. The simulator will create one row per person.</p>`;
      return;
    }
    root.innerHTML = list.map((member, index) => {
      ...
      return `<div class="v012-person" data-member="${index}">
        <div class="v012-person-title"><span style="background:${String(member.color || "#667085")}"></span><strong>${escapeHtml(member.name || `Person ${index + 1}`)}</strong><em>Person ${index + 1}</em></div>
  ...
  function renderEvents() {
    const root = document.getElementById("v012EventButtons");
    const scrubberWrap = document.getElementById("v012ScrubberWrap");
    if (!root || !scrubberWrap) return;
    const points = eventPoints();
    if (!points.length) {
      root.innerHTML = `<span class="v012-empty">Find a route first.</span>`;
    ...
    scrubberWrap.innerHTML = `<label class="v012-scrubber"><span>Journey scrubber</span><input id="v012Scrubber" type="range" min="${min}" max="${max}" step="30000" value="${current}"><small id="v012ScrubberLabel">${formatClock(current, false)}</small></label>`;
    const scrubber = document.getElementById("v012Scrubber");
    scrubber?.addEventListener("input", () => {
      const value = Number(scrubber.value);
      const label = document.getElementById("v012ScrubberLabel");
  ...
  function renderDiagnostics() {
    const root = document.getElementById("v012Diagnostics");
    if (!root) return;
  ...
  function render() {
    ...
    const banner = document.getElementById("v012TestBanner");
    ...
    const bannerTime = document.getElementById("v012BannerTime");
    ...
    const clock = document.getElementById("v012ClockNow");
    if (clock) clock.textContent = formatClock(now);
    const real = document.getElementById("v012ClockReal");
    if (real) real.textContent = `Real: ${formatClock(RealDate.now())}`;
    const speedLabel = document.getElementById("v012ClockSpeed");
    if (speedLabel) speedLabel.textContent = state.speed === 0 ? "PAUSED" : `${state.speed}×`;
    const datetime = document.getElementById("v012DateTime");
    if (datetime && document.activeElement !== datetime) datetime.value = formatLocalInput(now);
    const speed = document.getElementById("v012Speed");
    if (speed) speed.value = String(state.speed);
    const network = document.getElementById("v012Network");
    if (network) network.value = state.network;
    const networkNote = document.getElementById("v012NetworkNote");
  ...
  function installVersionTapUnlock() {
    const attach = () => {
      ...
      if (!version || version.dataset.v012Unlock) return Boolean(version);
      version.dataset.v012Unlock = "1";
