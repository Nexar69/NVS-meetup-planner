(() => {
  ...
  function loadTransferWatch0111() {
    if (!document.querySelector('link[data-transfer-watch-v0111="true"]')) {
      const link = document.createElement("link");
      link.rel = "stylesheet";
      link.href = "./transfer-watch-v0111.css";
      link.dataset.transferWatchV0111 = "true";
      document.head.appendChild(link);
    }
    if (!document.querySelector('script[data-transfer-watch-v0111="true"]')) {
      const script = document.createElement("script");
      script.src = "./transfer-watch-v0111.js";
      script.async = false;
      script.dataset.transferWatchV0111 = "true";
      document.body.appendChild(script);
    }
  }
  ...
  function loadOfflineJourney0111() {
    if (!document.querySelector('link[data-offline-journey-v0111="true"]')) {
      const link = document.createElement("link");
      link.rel = "stylesheet";
      link.href = "./offline-journey-v0111.css";
      link.dataset.offlineJourneyV0111 = "true";
      document.head.appendChild(link);
    }
    if (!document.querySelector('script[data-offline-journey-v0111="true"]')) {
      const script = document.createElement("script");
      script.src = "./offline-journey-v0111.js";
      script.async = false;
      script.dataset.offlineJourneyV0111 = "true";
      document.body.appendChild(script);
    }
  }
  ...
  function applyReleaseCopy() {
    ...
    if (liveNote) liveNote.innerHTML = `<strong>v0.11.1 hardens Meetup Intelligence.</strong> Realtime alerts distinguish early from late vehicles and impossible transfers, personal journeys proactively surface tight transfers and timetable-only stop awareness, Trip Mode adds voluntary quick check-ins and optional screen wake lock, Meetup Radar summarizes planned group convergence, the local What if? tool previews +5/+10 minute timing shifts without changing the shared plan, personal shared views keep a tab-scoped offline timetable fallback, shared links use a non-sliding backend expiry deadline, organizers can reset private personal check-in links without erasing visible check-in history, mobile PWA notifications are safer, and privacy-safe diagnostics make real-device bug reports easier.`;
  ...
  loadTransferWatch0111();
  loadStopAwareness0111();
  loadDiagnostics0111();
  loadMeetupRadar0111();
  loadWhatIf0111();
  loadOfflineJourney0111();
