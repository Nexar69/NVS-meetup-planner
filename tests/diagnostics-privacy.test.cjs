...
for (const forbidden of ["Secret Person", "Secret Home", "Secret Stop", "secret-plan-id", "secret-key", "53.6", "11.4", "geometry", "planId", "capabilityKey"]) {
  assert.equal(serialized.includes(forbidden), false, `diagnostics must exclude sensitive value/key: ${forbidden}`);
}
...
console.log("diagnostics-privacy: sanitized bug snapshot excludes names, coordinates, IDs, keys and location readings");
