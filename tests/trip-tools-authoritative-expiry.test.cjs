...
assert.match(tripToolsSource, /addEventListener\("nvs-shared-session-expired",\s*\(\)\s*=>\s*\{\s*invalidateCheckinUi\(\);\s*if\s*\(ownsForeground\(\)\)\s*\{[\s\S]*reconcileCheckinUiMessage\(\);[\s\S]*render\(\);[\s\S]*\}\s*\}\)/,
  'authoritative expiry should invalidate an in-flight Trip Tools generation and reconcile stale transient UI only with foreground ownership');
