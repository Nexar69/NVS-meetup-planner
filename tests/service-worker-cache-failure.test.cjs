  ...
  const context = {
    ...
    caches: {
      ...
      async keys() {
        ...
        return ["stale-cache", "meet-schwerin-v0.11.1-r13"];
      },
      async delete(key) {
...
(async () => {
  ...
  assert.match(source, /meet-schwerin-v0\.11\.1-r13/, "cache failure suite should exercise the current cache revision");
  ...
  console.log("service-worker-cache-failure: r13 install, online loads and activation survive CacheStorage failures without sacrificing older offline fallback or API privacy");
