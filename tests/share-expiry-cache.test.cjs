...
assert.match(legacyShareSource, /if \(generation !== deliveryGeneration\) return;/, "a superseded delivery must stop before invoking Web Share, clipboard, or prompt UI");
assert.match(legacyShareSource, /generation === deliveryGeneration\) showToast\(/, "late clipboard completion must not repaint toast UI after delivery invalidation");
assert.match(legacyShareSource, /generation === deliveryGeneration && error\?\.name !== "AbortError"/, "late native-share failures must not repaint superseded UI");
