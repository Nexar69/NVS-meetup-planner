...
// Route-derived voluntary intelligence must not mutate while frozen/hidden and
// must rely on the fresh Shared Live entry rather than introducing a second source.
assert.match(sources.voluntarySync, /function ownsLifecycle\(\) \{\s*return !lifecycleFrozen && !document\.hidden;/,
  "voluntary intelligence must share a visible lifecycle ownership boundary");
...
console.log("shared-trust-boundary-combined: revision + expiry + offline + visible-lifecycle contracts locked across Shared Live consumers");
