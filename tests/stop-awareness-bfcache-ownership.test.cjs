...
assert.match(source, /if \(lifecycleFrozen \|\| queued \|\| !recommendationsActive\) return;/, "MutationObserver work must not queue after pagehide");
assert.match(source, /if \(lifecycleFrozen \|\| !recommendationsActive\) return;/, "already queued observer work must lose authority after pagehide");
assert.match(source, /if \(lifecycleFrozen \|\| document\.hidden \|\| !recommendationsActive/, "timers and observers must stay stopped while frozen");
assert.match(source, /function freezeLifecycle\(\) \{[\s\S]*lifecycleFrozen = true;[\s\S]*clearTimeout\(timer\);[\s\S]*clearTimeout\(queuedTimer\);[\s\S]*observer\?\.disconnect\?\.\(\);[\s\S]*\}/, "pagehide should invalidate timer, queued render, and observer ownership without repainting the frozen DOM");
...
console.log("stop-awareness-bfcache-ownership: freeze, queued-work invalidation, fresh pageshow reconcile and privacy boundary passed");
