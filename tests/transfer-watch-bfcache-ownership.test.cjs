...
assert.match(source, /recommendationsActive = false;\s*clearTimeout\(timer\);\s*timer = null;\s*removeCard\(\);/s, "authoritative recommendation clear should cancel timers and remove stale visible guidance");
