...
const lifecycle = read("worker/src/lifecycle-entry.js");

assert.match(wrangler, /^name\s*=\s*"meet-schwerin"/m, "supported Cloudflare Worker name must remain meet-schwerin");
assert.match(wrangler, /^main\s*=\s*"src\/lifecycle-entry\.js"/m, "Worker entry point must use the lifecycle gateway relative to worker/");
...
assert.match(lifecycle, /import core from "\.\/entry\.js"/, "lifecycle gateway must retain the existing routing/shared-live Worker core");
assert.match(lifecycle, /authoritativeExpiry:\s*true/, "lifecycle gateway health contract must advertise authoritative expiry support");
assert.match(lifecycle, /expiration:/, "new shared sessions should be normalized to absolute KV expiration");
...
assert.match(readme, /Entry point: `src\/lifecycle-entry\.js`/, "deployment docs must identify the lifecycle gateway entry point");
...
console.log("worker-config: Cloudflare deployment and lifecycle contract is internally consistent");
