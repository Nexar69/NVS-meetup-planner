...
assert.match(tripTools, /function ownsForeground\(\)[\s\S]*return !lifecycleFrozen && !document\.hidden;/, "Trip Tools must centralize visible non-bfcache lifecycle ownership");
assert.match(tripTools, /function scheduleRender\(\)[\s\S]*clearTimeout\(timer\)[\s\S]*timer\s*=\s*null[\s\S]*if \(!ownsForeground\(\) \|\| !recommendationsActive\) return;/, "Trip Tools scheduler must stay stopped while bfcache-frozen, hidden or recommendations are empty, including foreground/pageshow restarts");
assert.match(tripTools, /timer = setTimeout\(\(\) => \{\s*if \(!ownsForeground\(\)\) return;[\s\S]*scheduleRender\(\);\s*\}, 15_000\)/, "a route-age callback that escapes cancellation must independently reject hidden/frozen execution before repainting or rearming");
...
assert.match(tripTools, /addEventListener\("nvs-group-recommendations-rendered"[\s\S]*recommendationsActive\s*=\s*true[\s\S]*lastRouteUpdate\s*=\s*Date\.now\(\)[\s\S]*if \(!ownsForeground\(\)\) return;[\s\S]*render\(\)[\s\S]*scheduleRender\(\)/, "fresh recommendations may update memory-only route ownership while suspended but must repaint/restart only with foreground ownership");
assert.match(tripTools, /lastRouteUpdate\s*>\s*0\s*\?\s*`Routes \$\{relativeAge\(lastRouteUpdate\)\}`\s*:\s*"No active route"/, "cleared route-age state must render as an explicit inactive state instead of a bogus 1970-age timestamp");
...
assert.match(tripTools, /if \(!ownsForeground\(\) \|\| generation !== wakeRequestGeneration \|\| !wakeWanted \|\| !tripDialog\(\)\?\.open\)[\s\S]*await lock\?\.release\?\.\(\)/, "a late wake lock must be released instead of adopted after clear, backgrounding, lifecycle loss, or dialog close");
