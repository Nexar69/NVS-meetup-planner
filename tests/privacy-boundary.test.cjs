...
assert.match(tripTools, /addEventListener\("nvs-group-recommendations-rendered"[\s\S]*lastRouteUpdate\s*=\s*Date\.now\(\)[\s\S]*render\(\)[\s\S]*scheduleRender\(\)/, "fresh recommendations must restart the Trip Tools refresh scheduler after an empty-state teardown");
