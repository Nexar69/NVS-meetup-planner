...
assert.match(tripTools, /function scheduleRender\(\)[\s\S]*clearTimeout\(timer\)[\s\S]*timer\s*=\s*null[\s\S]*if \(lifecycleFrozen \|\| document\.hidden\) return;[\s\S]*if \(!recommendationsActive\) return;/, "Trip Tools scheduler must stay stopped while bfcache-frozen, hidden or recommendations are empty, including foreground/pageshow restarts");
...
assert.match(tripTools, /addEventListener\("nvs-group-recommendations-rendered"[\s\S]*recommendationsActive\s*=\s*true[\s\S]*lastRouteUpdate\s*=\s*Date\.now\(\)[\s\S]*if \(lifecycleFrozen\) return;[\s\S]*render\(\)[\s\S]*scheduleRender\(\)/, "fresh recommendations must update route ownership while frozen but only repaint/restart Trip Tools after the lifecycle reopens");
