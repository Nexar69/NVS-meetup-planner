...
assert.match(tripTools, /addEventListener\("nvs-group-recommendations-rendered"[\s\S]*recommendationsActive\s*=\s*true[\s\S]*lastRouteUpdate\s*=\s*Date\.now\(\)[\s\S]*render\(\)[\s\S]*scheduleRender\(\)/, "fresh recommendations must reactivate and restart the Trip Tools refresh scheduler after an empty-state teardown");
