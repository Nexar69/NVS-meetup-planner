...
function runtime(fetchImpl = async () => new Response("network", { status: 200 }), { fastTimeout = false, currentShellReady = true, clientListFails = false, focusFails = false, openWindowFails = false } = {}) {
  ...
  const context = {
    ...
    caches: {
      ...
      async keys() { return ["old-cache", "meet-schwerin-v0.11.1-r13", "meet-schwerin-v0.11.1-r14", "meet-schwerin-v0.11.1-r15", "meet-schwerin-v0.11.1-r16", "meet-schwerin-v0.11.1-r17"]; },
...
(async () => {
  ...
  }
  {
    ...
    assert.deepEqual(rt.deleted, ["old-cache", "meet-schwerin-v0.11.1-r13", "meet-schwerin-v0.11.1-r14", "meet-schwerin-v0.11.1-r15", "meet-schwerin-v0.11.1-r16"], "healthy r17 shell should clean older caches while preserving itself");
  ...
  assert.match(originalSource, /^const CACHE_NAME = "meet-schwerin-v0\.11\.1-r17";/, "service-worker behavior suite should exercise the current cache revision");
  ...
  console.log("service-worker-behavior: privacy, offline, guarded install/activation, slow-network/transient-server fallback, resilient notification taps and r17 app-shell behavior passed");
